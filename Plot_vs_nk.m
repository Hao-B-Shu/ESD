...
colorbar;
xlabel('$n$'); ylabel('$k$');
title(sprintf('\\textbf{SVSD with respect to $n,k$, where $t=10^{%d}$}',log10(t)));
axis xy;
set(h1,'AlphaData',~isnan(SESD));
cb = colorbar;
cb.Ticks = -24:4:0; 
cb.TickLabels = arrayfun(@(x) sprintf('10^{%d}', x), cb.Ticks, 'UniformOutput', false);
name=sprintf('SVSD10%d.eps', log10(t));
...
colorbar;
xlabel('$n$'); ylabel('$k$');
title(sprintf('\\textbf{NVSR with respect to $n,k$, where $t=10^{%d}$}',log10(t)));
axis xy;
set(h2,'AlphaData',~isnan(SESD));
cb = colorbar;
cb.Ticks = -8:2:0;
cb.TickLabels = arrayfun(@(x) sprintf('10^{%d}', x), cb.Ticks, 'UniformOutput', false);
name=sprintf('NVSR10%d.eps', log10(t));
filepath = fullfile(saveDir, name);
print(gcf, '-depsc', filepath);
...
grid on;
view([1 1 0.5]);
name=sprintf('QBER10%d.eps', log10(t));
filepath = fullfile(saveDir, name);
print(gcf, '-depsc', filepath);