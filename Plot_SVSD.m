...
if isnan(k)
   k = floor(n/2);%k = n;
   klabel = '$k=\left\lfloor\frac{n}{2}\right\rfloor$';%klabel = '$k=n$';   
else
   k=ones(size(n))*k;
...
figure; hold on;
h = gobjects(2,1);
h(1) = plot(n, a, 'DisplayName', 'Non-vacuum signal sifting rate: $P_{\geq k|n}$');
h(2) = plot(n, b, 'DisplayName', 'Vacuum signal sifting rate: $Q_{\geq k|n}$');
hold off;
axis([n(1) nmax 0 1])
...
grid on;
name=sprintf('SVSD%g.eps', P);%name=sprintf('SVSDk=n%g.eps', P);
